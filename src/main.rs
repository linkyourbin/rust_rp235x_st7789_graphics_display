...
use embedded_graphics::mono_font::ascii::{FONT_6X10, FONT_10X20}; // Standard ASCII fonts for text rendering
use embedded_graphics::pixelcolor::Rgb565; // 16-bit RGB color format (5-6-5)
use embedded_graphics::prelude::*;
use embedded_graphics::primitives::{Circle, Line, PrimitiveStyle, Rectangle}; // Basic drawing primitives
...
use mipidsi::options::{Orientation, Rotation}; // Display orientation options // Display initialization builder
...
static SPI_BUS: static_cell::StaticCell<
    Mutex<NoopRawMutex, RefCell<Spi<'static, embassy_rp::peripherals::SPI1, Blocking>>>,
    ...
    T::Error: core::fmt::Debug,
{
    ...
    Text::new("Text within bounds", Point::new(20, 160), small_text_style)
        .draw(display)
        .unwrap();
    ...
    T::Error: core::fmt::Debug,
{
    ...
    for _ in 0..100 {
        ...
        for i in 0..8 {
            ...
            Line::new(Point::new(120, 120), Point::new(x as i32, y as i32))
    ...
    T::Error: core::fmt::Debug,
{
    ...
    Text::new("EMBEDDED GRAPHICS", Point::new(10, 20), small_text_style)
        .draw(display)
        .unwrap();

    Text::new("IMAGE TEST", Point::new(80, 220), small_text_style)
        .draw(display)
        .unwrap();
}
    ...
    T::Error: core::fmt::Debug,
{
    ...
    for &color in colors.iter() {
        ...
        // Display test title
        Text::new("FILL TEST", Point::new(10, 50), text_style)
            .draw(display)
            .unwrap();

        // Display "TIME: " label
        Text::new("TIME: ", Point::new(10, 100), text_style)
            .draw(display)
            .unwrap();
        ...
        if let Ok(time_str) = core::str::from_utf8(&time_str[..5]) {
            Text::new(time_str, Point::new(80, 100), text_style)
                .draw(display)
                .unwrap();
        }

        // Display test identification label
        Text::new("SCREEN FILL TEST", Point::new(10, 200), small_text_style)
            .draw(display)
            .unwrap();
...
async fn main(_spawner: Spawner) {
    ...
    let raw_image_11 =
        ImageRawLE::<Rgb565>::new(include_bytes!("../assets/mode_6/xiaohei.raw"), 240);
    let im11 = Image::new(&raw_image_11, Point::new(0, 0));

    let raw_image_12 =
        ImageRawLE::<Rgb565>::new(include_bytes!("../assets/mode_6/xiaohei_cute.raw"), 240);
    let im12 = Image::new(&raw_image_12, Point::new(0, 0));

    let raw_image_13 =
        ImageRawLE::<Rgb565>::new(include_bytes!("../assets/mode_6/classic.raw"), 240);
    let im13 = Image::new(&raw_image_13, Point::new(0, 0));
